 ...
 * 1. Cria tabela invite_codes
 * 2. Insere 170 códigos
 * 3. Verifica inserção
 */
...
async function main() {
  console.log('\n╔════════════════════════════════════════════════════════════════╗');
  console.log('║   🚀 SETUP AUTOMÁTICO COMPLETO - 170 CÓDIGOS DUA IA           ║');
  console.log('╚════════════════════════════════════════════════════════════════╝\n');

  try {
    // PASSO 1: Criar tabela via SQL direto
    console.log('📋 PASSO 1: Criando tabela invite_codes...\n');
    ...
    try {
      // Tentar criar via SQL direto (pode falhar se não tiver RPC)
      await supabase.from('invite_codes').select('code').limit(1);
      console.log('   ✅ Tabela invite_codes já existe\n');
    } catch (e) {
      console.log('   ⚠️  Tabela não existe, mas não podemos criar via API');
      console.log('   ℹ️  Por favor, execute manualmente no SQL Editor:');
      console.log('   ' + createTableSQL.replace(/\n/g, '\n   '));
      console.log('\n   Depois execute este script novamente.\n');
      return;
    }

    // PASSO 2: Inserir códigos
    console.log('📥 PASSO 2: Inserindo 170 códigos...\n');

    const codesData = CODIGOS_170.map(code => ({
      code,
      active: true,
      used_by: null,
    ...
    for (let i = 0; i < codesData.length; i += batchSize) {
      ...
      console.log(`   Lote ${batchNum}/4: Inserindo ${batch.length} códigos...`);
      ...
      if (error) {
        ...
        console.log(`   ✅ Lote ${batchNum}: ${inserted} códigos inseridos`);
      }
    }

    console.log(`\n✅ Total inserido/atualizado: ${totalInserido} códigos\n`);

    // PASSO 3: Verificação
    console.log('🔍 PASSO 3: Verificação final...\n');

    const { data: codigos, count } = await supabase
      .from('invite_codes')
      .select('code', { count: 'exact' })
      .eq('active', true);

    console.log(`📊 Estatísticas:`);
    console.log(`   • Total de códigos ativos: ${count || 0}`);
    console.log(`   • Códigos esperados: 170`);
    console.log(`   • Status: ${count >= 170 ? '✅ SUCESSO' : '⚠️  INCOMPLETO'}\n`);

    if (count >= 170) {
      console.log('╔════════════════════════════════════════════════════════════════╗');
      console.log('║   ✅ SISTEMA COMPLETO! 170 CÓDIGOS PRONTOS PARA USO           ║');
      console.log('╚════════════════════════════════════════════════════════════════╝\n');
    }
