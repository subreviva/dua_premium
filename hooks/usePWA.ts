...
export function usePWA(): PWAHookReturn {
    ...
    if ('serviceWorker' in navigator) {
      try {
        ...
        registration.addEventListener('updatefound', () => {
          // PRODUCTION: Removed console.log('Nova versão do SW disponível');
          audit.custom('sw_update_available', { version: 'v1.2.0' });
        });

        // PRODUCTION: Removed console.log('Service Worker registrado:', registration);
        audit.custom('sw_registered', { scope: registration.scope });
      } catch (error) {
        // PRODUCTION: Removed console.error('Falha ao registrar SW:', error);
  ...
  const handleAppInstalled = () => {
    ...
    // PRODUCTION: Removed console.log('PWA instalada com sucesso!');
  };
