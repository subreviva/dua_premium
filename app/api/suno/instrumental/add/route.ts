...
export async function POST(request: NextRequest) {
  try {
    ...
    const requiredParams = ["uploadUrl", "title", "tags", "negativeTags"]
    ...
    // Call Suno API
    const result = await client.addInstrumental(body)

    return NextResponse.json(result)
  } catch (error) {
    console.error("[v0] Error adding instrumental:", error)
    ...
    return NextResponse.json(
      {
        code: 500,
        msg: "Failed to add instrumental",
        data: null,
      },
      { status: 500 },
    )
