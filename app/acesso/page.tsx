...
import { RedeemDialog } from "@/components/ui/redeem-dialog";
...
export default function AcessoPage() {
  ...
  const [showRedeemDialog, setShowRedeemDialog] = useState(true);
  ...
  const handleValidateCode = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (error || !data) {
        ...
        throw new Error("Código não existe");
      }
      if (!data.active) {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: profileData, error: profileError } = await supabase.from('users').insert({
        ...
        duaia_credits: 0,
        duacoin_balance: 0,
        creditos_servicos: 150, // ⚡ CRÍTICO: Créditos de serviços (SOURCE OF TRUTH)
      ...
      if (profileError) {
        ...
        // ⚡ CRÍTICO: Se falhar, tentar UPDATE em vez de INSERT
        console.log('[REGISTER] Tentando UPDATE em vez de INSERT...');
        const { data: updateData, error: updateError } = await supabase
          ...
          .update({
            ...
            duaia_credits: 0,
            duacoin_balance: 0,
            creditos_servicos: 150, // ⚡ GARANTIR que créditos sejam adicionados
            saldo_dua: 50,
            account_type: 'normal',
          })
          .eq('id', userId)
        ...
        if (updateError) {
          console.error('[REGISTER] ❌ Erro UPDATE:', updateError);
          toast.error("Aviso", {
            description: "Perfil criado mas pode haver problemas. Contacte o suporte.",
            duration: 5000,
          });
        } else {
          console.log('[REGISTER] ✅ Perfil atualizado via UPDATE:', updateData);
        }
      } else {
        console.log('[REGISTER] ✅ Perfil criado via INSERT:', profileData);
      }
      ...
      // PASSO 5: Marcar código como usado COM VERIFICAÇÃO CRÍTICA
      if (validatedCode) {
        console.log('[REGISTER] Verificando código antes de marcar como usado...');
        ...
        console.log('[REGISTER] ✅ Código marcado como usado com sucesso');
      }
        ...
        {step === "code" ? (
          <>
            ...
            <div className="text-center mb-8 sm:mb-12">
              <motion.h1 
                ...
                className="text-4xl sm:text-6xl font-light tracking-tight text-white mb-4"
              ...
              <motion.p 
                ...
                className="text-neutral-500 text-xs sm:text-sm font-light tracking-wide"
              >
                Insira seu código de acesso exclusivo
              </motion.p>
            </div>
          ...
          <>
            ...
            <div className="flex items-center justify-center gap-3 mb-8">
              <div className="w-2 h-0.5 bg-neutral-800" />
            ...
            <motion.div 
              ...
              className="bg-neutral-950/40 backdrop-blur-2xl rounded-none p-8 sm:p-12 shadow-2xl shadow-black/50"
            >
              <motion.form 
                initial={{ opacity: 0, x: 20 }} 
                animate={{ opacity: 1, x: 0 }} 
            ...
            {/* Minimal footer */}
            <p className="text-center text-neutral-800 text-xs mt-8 font-light tracking-wide">
              DUA 2025
            </p>
          </>
        )}
