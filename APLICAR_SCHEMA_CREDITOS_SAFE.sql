-- =====================================================
-- SCHEMA ULTRA RIGOROSO - SISTEMA DE CRÉDITOS (SAFE MODE)
-- =====================================================
-- Este script cria/atualiza toda a estrutura necessária
-- para o sistema de créditos funcionar 100%
-- Versão SAFE: Remove políticas existentes antes de criar
-- =====================================================
...
-- Tornar access_code único (se já existe dado duplicado, vai falhar - ignorar erro)
DO $$ 
BEGIN
  ALTER TABLE public.users ADD CONSTRAINT users_access_code_key UNIQUE (access_code);
EXCEPTION
...
-- admin_accounts (TODAS as variações)
...
DROP POLICY IF EXISTS "users_can_check_admin_status" ON public.admin_accounts;
...
-- 11. CRIAR FUNÇÃO PARA ATUALIZAR CRÉDITOS (MAIS SIMPLES)
-- =====================================================
...
BEGIN
  ...
  RAISE NOTICE '📊 Tabelas criadas: users (atualizada), credit_transactions, credit_packages, admin_accounts';
  ...
  RAISE NOTICE '🎯 Políticas removidas e recriadas para evitar conflitos';
END $$;
