...
function FeatureCard({ children, step }: { children: React.ReactNode; step: number }) {
  ...
  return (
    ...
    >
      <div className="relative w-full overflow-hidden rounded-3xl border border-white/[0.12] bg-black/40 backdrop-blur-xl transition-all duration-500 hover:border-white/[0.2] hover:bg-black/50 hover:shadow-2xl hover:shadow-white/5">
        <div className="m-10 min-h-[450px] w-full">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <motion.h2
                className="text-2xl font-bold tracking-tight text-white md:text-3xl drop-shadow-lg"
                initial={{ opacity: 0, x: -20 }}
                animate={{ opacity: 1, x: 0 }}
              ...
              >
                <p className="text-base leading-relaxed text-white/70 font-light drop-shadow-md">
                    ...
                    return (
                        <motion.li key={step.name} initial="inactive" animate={isCurrent ? "active" : "inactive"} variants={stepVariants} transition={{ duration: 0.3 }} className="relative" >
                            <button
                                type="button"
                                className={cn(
                                    "group flex items-center gap-2.5 rounded-full px-4 py-2 text-sm font-medium transition-all duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-white/20 backdrop-blur-md",
                                    isCurrent 
                                        ? "bg-white/[0.18] text-white border border-white/30 shadow-lg" 
                                        : "bg-black/[0.3] text-white/70 hover:bg-black/[0.4] border border-white/[0.12] hover:border-white/[0.2]"
                                )}
                            ...
                            >
                                <span className={cn(
                                    ...
                                    isCompleted 
                                        ...
                                        : isCurrent 
                                            ? "bg-white/20 text-white" 
                                            : "bg-white/[0.06] text-white/50 group-hover:bg-white/[0.12]"
                                )}>
                                    {isCompleted ? (
...
const defaultClasses = {
  img: "rounded-2xl border border-white/[0.08] shadow-2xl shadow-black/20 backdrop-blur-sm",
